...
export const name = 'typescript.diagnostics-problems-panel-class-before-declaration'
...
export const test: Test = async ({ expect, FileSystem, Locator, Main, Panel, Problems, Settings, Workspace }) => {
  ...
  // act
  await Main.openUri(`${workspaceUrl}/src/class-before-declaration.ts`)
  ...
  await expect(label).toHaveText(`Class 'Later' used before its declaration.`)
}
