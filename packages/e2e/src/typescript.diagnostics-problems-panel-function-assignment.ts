...
export const name = 'typescript.diagnostics-problems-panel-function-assignment'
...
export const test: Test = async ({ expect, FileSystem, Locator, Main, Panel, Problems, Settings, Workspace }) => {
  ...
  // act
  await Main.openUri(`${workspaceUrl}/src/function-assignment.ts`)
  ...
  await expect(label).toHaveText(`Cannot assign to 'run' because it is a function.`)
}
