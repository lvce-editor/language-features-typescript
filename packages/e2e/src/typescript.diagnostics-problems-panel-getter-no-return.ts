...
export const name = 'typescript.diagnostics-problems-panel-getter-no-return'
...
export const test: Test = async ({ expect, FileSystem, Locator, Main, Panel, Problems, Settings, Workspace }) => {
  ...
  // act
  await Main.openUri(`${workspaceUrl}/src/getter-no-return.ts`)
  ...
  await expect(label).toHaveText(`A 'get' accessor must return a value.`)
}
