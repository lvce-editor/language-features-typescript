...
export const name = 'typescript.diagnostics-problems-panel-mixed-bigint-number'
...
export const test: Test = async ({ expect, FileSystem, Locator, Main, Panel, Problems, Settings, Workspace }) => {
  ...
  // act
  await Main.openUri(`${workspaceUrl}/src/mixed-bigint-number.ts`)
  ...
  await expect(label).toHaveText(`Operator '+' cannot be applied to types '1n' and '1'.`)
}
