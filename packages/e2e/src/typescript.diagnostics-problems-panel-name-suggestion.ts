...
export const name = 'typescript.diagnostics-problems-panel-name-suggestion'
...
export const test: Test = async ({ expect, FileSystem, Locator, Main, Panel, Problems, Settings, Workspace }) => {
  ...
  // act
  await Main.openUri(`${workspaceUrl}/src/name-suggestion.ts`)
  ...
  await expect(label).toHaveText(`Cannot find name 'count'. Did you mean 'counter'?`)
}
