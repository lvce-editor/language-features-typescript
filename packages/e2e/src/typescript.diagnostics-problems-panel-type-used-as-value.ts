...
export const name = 'typescript.diagnostics-problems-panel-type-used-as-value'
...
export const test: Test = async ({ expect, FileSystem, Locator, Main, Panel, Problems, Settings, Workspace }) => {
  ...
  // act
  await Main.openUri(`${workspaceUrl}/src/type-used-as-value.ts`)
  ...
  await expect(label).toHaveText(`'Person' only refers to a type, but is being used as a value here.`)
}
