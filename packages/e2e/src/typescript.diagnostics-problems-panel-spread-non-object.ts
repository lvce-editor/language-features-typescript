...
export const name = 'typescript.diagnostics-problems-panel-spread-non-object'
...
export const test: Test = async ({ expect, FileSystem, Locator, Main, Panel, Problems, Settings, Workspace }) => {
  ...
  // act
  await Main.openUri(`${workspaceUrl}/src/spread-non-object.ts`)
  ...
  await expect(label).toHaveText('Spread types may only be created from object types.')
}
