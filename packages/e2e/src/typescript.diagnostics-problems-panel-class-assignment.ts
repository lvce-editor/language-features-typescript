...
export const name = 'typescript.diagnostics-problems-panel-class-assignment'
...
export const test: Test = async ({ expect, FileSystem, Locator, Main, Panel, Problems, Settings, Workspace }) => {
  ...
  // act
  await Main.openUri(`${workspaceUrl}/src/class-assignment.ts`)
  ...
  await expect(label).toHaveText(`Cannot assign to 'Item' because it is a class.`)
}
