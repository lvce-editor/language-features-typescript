...
export const name = 'typescript.diagnostics-problems-panel-invalid-index-type'
...
export const test: Test = async ({ expect, FileSystem, Locator, Main, Panel, Problems, Settings, Workspace }) => {
  ...
  // act
  await Main.openUri(`${workspaceUrl}/src/invalid-index-type.ts`)
  ...
  await expect(label).toHaveText(`Type 'String' cannot be used as an index type.`)
}
