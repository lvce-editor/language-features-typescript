...
export const name = 'typescript.diagnostics-problems-panel-abstract-member'
...
export const test: Test = async ({ expect, FileSystem, Locator, Main, Panel, Problems, Settings, Workspace }) => {
  ...
  // act
  await Main.openUri(`${workspaceUrl}/src/abstract-member.ts`)
  ...
  await expect(label).toHaveText(
    `Non-abstract class 'Circle' does not implement inherited abstract member area from class 'Shape'.`,
  )
}
