...
export const name = 'typescript.diagnostics-problems-panel-value-used-as-type'
...
export const test: Test = async ({ expect, FileSystem, Locator, Main, Panel, Problems, Settings, Workspace }) => {
  ...
  // act
  await Main.openUri(`${workspaceUrl}/src/value-used-as-type.ts`)
  ...
  await expect(label).toHaveText(
    `'Profile' refers to a value, but is being used as a type here. Did you mean 'typeof Profile'?`,
  )
}
